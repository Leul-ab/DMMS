    ...
    return (
        <button
            ...
            className={[
                ...
                isMobile
                    ? isExpanded
                        ? 'left-[calc(var(--sidebar-width,18rem)-2px)]'
                        : 'left-0'
                    : isExpanded
                      ? 'left-[calc(var(--sidebar-width,16rem)-2px)]'
                      : 'left-[calc(var(--sidebar-width-icon,3rem)+6px)]',
            ].join(' ')}
            style={{
                // Use CSS var so it animates in sync with the sidebar transition
                transition:
                    'left 300ms ease-in-out, width 200ms, background-color 150ms, box-shadow 150ms',
            }}
        >
            {isExpanded ? (
