...
export default function AllBookingsSidebar() {
    ...
    return (
        <>
            ...
            {/* Sidebar */}
            <div
                className={`fixed left-0 top-0 z-50 h-full w-full max-w-md transform border-r border-gray-200 bg-white shadow-2xl transition-transform duration-300 ${
                ...
                <div className="flex h-[calc(100%-64px)] flex-col overflow-y-auto">
                    {isLoading ? (
                        ...
                        <div className="flex flex-1 flex-col items-center justify-center p-8 text-center">
                            ...
                            <p className="mt-1 text-sm text-gray-400">There are no bookings to display.</p>
                        </div>
                    ) : (
                        <div className="p-4 space-y-4">
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                <div className="rounded-xl bg-indigo-50 p-3 text-center">
                                    <p className="text-2xl font-black text-indigo-600">{data.total}</p>
                                    <p className="text-xs font-semibold text-indigo-500">Total</p>
                                </div>
                                <div className="rounded-xl bg-green-50 p-3 text-center">
                                    <p className="text-2xl font-black text-green-600">{data.active_count}</p>
                                    <p className="text-xs font-semibold text-green-500">Active</p>
                                </div>
                                <div className="rounded-xl bg-red-50 p-3 text-center">
                                    <p className="text-2xl font-black text-red-600">{data.total - data.active_count}</p>
                                    <p className="text-xs font-semibold text-red-500">Inactive</p>
                                </div>
                            </div>
                            ...
                            {activeBookings.length > 0 && (
                                <div>
                                    <h3 className="mb-3 text-sm font-bold uppercase tracking-wider text-green-600 flex items-center gap-2">
                                    ...
                                    <div className="space-y-3">
                                        {activeBookings.map((booking) => (
                                            <BookingCard
                                                key={booking.id}
                                                booking={booking}
                                                timeRemaining={timeRemainingMap[booking.id]}
...
function BookingCard({ booking, timeRemaining, isExpanded, onToggle, getStatusBadge, formatTime, formatDate }: BookingCardProps) {
    const isActive = booking.status === 'active' && !booking.is_expired;

    return (
        <div
            className={`rounded-xl border bg-white shadow-sm transition-all ${
                isActive ? 'border-green-200 ring-1 ring-green-100' : 'border-gray-100'
            }`}
        >
            ...
            >
                <div className={`flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg ${
                    isActive ? 'bg-green-100' : booking.status === 'cancelled' ? 'bg-red-100' : 'bg-gray-100'
                }`}>
                    <User className={`h-5 w-5 ${
                        isActive ? 'text-green-600' : booking.status === 'cancelled' ? 'text-red-500' : 'text-gray-500'
                    }`} />
                </div>
                <div className="min-w-0 flex-1">
                    <p className="font-bold text-gray-900 truncate">{booking.customer_name}</p>
                    <div className="flex items-center gap-2 mt-0.5">
                        <Table2 className="h-3 w-3 text-gray-400" />
                        <span className="text-xs text-gray-500">
                            {booking.tables.map(t => `Table ${t.table_number}`).join(', ')}
                        </span>
                    </div>
                </div>
            ...
            {isExpanded && (
                <div className="border-t border-gray-50 px-4 pb-4 pt-3 space-y-3">
                    ...
                    {/* Booked Tables */}
                    <div>
                        <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Tables</p>
                        <div className="flex flex-wrap gap-1.5">
                            {booking.tables.map((table) => (
                                <Badge key={table.id} variant="secondary" className="bg-indigo-50 text-indigo-700 hover:bg-indigo-50">
                    ...
                    {booking.expires_at && (
                        <div className="flex items-center gap-2 text-sm">
                            ...
                            <span className={`font-medium ${isActive ? 'text-orange-600' : 'text-gray-500'}`}>
                                {formatDate(booking.expires_at)} at {formatTime(booking.expires_at)}
                            </span>
                        </div>
                    )}
                    ...
                    {booking.cancelled_at && (
                        <div className="flex items-center gap-2 text-sm">
                            ...
                            <span className="font-medium text-red-600">
                                {formatDate(booking.cancelled_at)} at {formatTime(booking.cancelled_at)}
                            </span>
                        </div>
                    )}
