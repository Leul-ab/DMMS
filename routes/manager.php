...
Route::middleware(['auth'])
    ...
    ->group(function () {
        ...
        )->name('branches.index')->middleware('permission:view branches');

        Route::get(
            'branches/create',
            [BranchController::class, 'create']
        )->name('branches.create')->middleware('permission:create branches');

        Route::post(
            'branches',
            [BranchController::class, 'store']
        )->name('branches.store')->middleware('permission:create branches');

        Route::get(
            'branches/{branch}',
            [BranchController::class, 'show']
        )->name('branches.show')->middleware('permission:view branches');

        Route::get(
            'branches/{branch}/edit',
            [BranchController::class, 'edit']
        )->name('branches.edit')->middleware('permission:update branches');

        Route::put(
            'branches/{branch}',
            [BranchController::class, 'update']
        )->name('branches.update')->middleware('permission:update branches');

        Route::delete(
            'branches/{branch}',
            [BranchController::class, 'destroy']
        )->name('branches.destroy')->middleware('permission:delete branches');
    });
...
Route::middleware(['auth'])
    ...
    ->group(function () {
        ...
        // Branch switch (assigned branches only)
        Route::post(
            'branches/switch',
            [BranchContextController::class, 'switch']
        )->name('branches.switch')->middleware('permission:switch branches');
        ...
        Route::patch('categories/{category}/toggle-status', [MenuCategoryController::class, 'toggleStatus'])
        ...
        Route::patch('items/{item}/toggle-availability', [MenuItemController::class, 'toggleAvailability'])
            ->name('items.toggle-availability')->middleware('permission:status menu items');
        Route::patch('items/{item}/price', [MenuItemController::class, 'updatePrice'])
            ->name('items.update-price')->middleware('permission:update menu items');
        Route::post('items/{item}/image', [MenuItemController::class, 'uploadImage'])
        ...
        Route::patch('orders/{order}/verify-payment', [OrderController::class, 'verifyPayment'])
        ...
        Route::patch('customers/{customer}/toggle-membership', [CustomerController::class, 'toggleMembership'])
        ...
        Route::post('bookings/{booking}/complete', [BookingManagementController::class, 'complete'])
